...
def sane_path(path):
    Path(path).resolve(strict=True)  # Resolves the path and checks if it exists (raises an error if not)
...
def get_log_body(log_index, debug=False):
    assert sane_index(log_index), "The value is Not a Number (NaN)."  # Validate that the log index is a number
    ...
    body = next(iter(data.values()))['body']  # Extract the 'body' field from the log entry
    body = json.loads(base64.b64decode(body))  # Decode the body from base64
    return body

# Fetches a full log entry by its index
def get_log_entry(log_index, debug=False):
    assert sane_index(log_index), "The value is Not a Number (NaN)."  # Validate log index
    api = f'https://rekor.sigstore.dev/api/v1/log/entries?logIndex={log_index}'
...
def inclusion(log_index, artifact_filepath, debug=False):
    ...
    signature = base64.b64decode(log['spec']['signature']['content'])  # Decode the signature from base64
    cert = base64.b64decode(log['spec']['signature']['publicKey']['content'])  # Decode the public key
    public_key = extract_public_key(cert)  # Extract the public key from the certificate
    verify_artifact_signature(signature, public_key, artifact_filepath)  # Verify the artifact signature
    ...
    verify_inclusion(DefaultHasher, proof['logIndex'], proof['treeSize'], proof['leafHash'], proof['hashes'], proof['rootHash'])
...
def consistency(prev_checkpoint, debug=False):
    assert prev_checkpoint != {}, 'Previous checkpoint empty'  # Ensure the previous checkpoint is not empty
    ...
    proof = requests.get(f'https://rekor.sigstore.dev/api/v1/log/proof?firstSize={prev_checkpoint["treeSize"]}&lastSize={treeSize}').json()['hashes']

    # Verify the consistency between the previous and the latest checkpoint using the Merkle proof
    verify_consistency(DefaultHasher, prev_checkpoint['treeSize'], treeSize, proof, prev_checkpoint['rootHash'], rootHash)
